...
const catFoodApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getCatFoods: builder.query({
      query: (search) => ({
        url: `https://pet-food-server.onrender.com/api/v1/cat-foods?search=${search}`,
      }),
      providesTags:['foods']
    }),
    getCatFoodsQueries: builder.query({
      query: (shop) => ({
        url: `https://pet-food-server.onrender.com/api/v1/cat-foods-query?shop=${shop}`,
      }),
      providesTags:["foods"]
    }),
  ...
  sellerAddFood:builder.query({
    query:(email)=>({
      url:`https://pet-food-server.onrender.com/api/seller/foods?email=${email}`
    }),
    providesTags:['foods']
  }),
